  ...
  try {
    ...
    const enrichedEvents = await Promise.all(
      events.map(async (event) => {
        ...
        return { ...event, attendeeDetails: attendeeDetails };
      })
    );
  ...
  if (body.eventId) {
    ...
    try {
      ...
      if (attendeeIndex > -1) {
        ...
        if (event.isLimitEnabled && typeof event.attendanceLimit === 'number' && event.attendeesCounter.length >= event.attendanceLimit) {
            return NextResponse.json({ message: "Event attendance limit reached." }, { status: 409 });
        }
    ...
    const { title, date, description, location, imageUrl, attendees, cardColor, isLimitEnabled, attendanceLimit, cardEnabled, inDescription } = body;
...
export async function PUT(request) {
    ...
    try {
        ...
        if (updatedEventData.resetAttendees === true) {
        ...
        const dataToMerge = {
            ...updatedEventData,
            attendanceLimit: updatedEventData.hasOwnProperty('isLimitEnabled') ? (updatedEventData.isLimitEnabled ? updatedEventData.attendanceLimit : null) : originalEvent.attendanceLimit,
            cardEnabled: typeof updatedEventData.cardEnabled === 'boolean' ? updatedEventData.cardEnabled : originalEvent.cardEnabled,
            inDescription: Array.isArray(updatedEventData.inDescription) ? updatedEventData.inDescription : originalEvent.inDescription,
        };
        delete dataToMerge.resetAttendees;
        const finalUpdatedEvent = { ...originalEvent, ...dataToMerge, attendeesCounter: finalAttendeesCounter };
...
export async function PATCH(request) {
    ...
    const isAdmin = sessionClaims?.metadata?.admin === true;
    const isCommittee = sessionClaims?.metadata?.committee === true;
    if (!isAdmin && !isCommittee) return NextResponse.json({ message: "Forbidden: User cannot verify attendees." }, { status: 403 });
    let body;
    try {
        body = await request.json();
        if (!body.eventId || !body.attendeeUserId || typeof body.verified !== 'boolean') {
            throw new Error("Missing required fields: eventId, attendeeUserId, verified (boolean)");
        }
    } catch (error) { return NextResponse.json({ message: "Invalid request body", error: error.message }, { status: 400 }); }
    const { eventId, attendeeUserId, verified } = body;
    try {
        const events = await readEvents();
        const eventIndex = events.findIndex((e) => e.id === eventId);
        if (eventIndex === -1) return NextResponse.json({ message: "Event not found" }, { status: 404 });
        const event = events[eventIndex];
        if (!Array.isArray(event.attendeesCounter)) {
            return NextResponse.json({ message: "Attendees list not found or invalid for this event." }, { status: 400 });
        }
        const attendeeIndex = event.attendeesCounter.findIndex(att => att.userID === attendeeUserId);
        if (attendeeIndex === -1) {
            return NextResponse.json({ message: "Attendee not found in this event." }, { status: 404 });
        }
        event.attendeesCounter[attendeeIndex].verified = verified;
        await writeEvents(events);
        return NextResponse.json(event.attendeesCounter[attendeeIndex], { status: 200 });
    } catch (error) {
        return NextResponse.json({ message: error.message || "Failed to update verification status" }, { status: 500 });
    }
}
