...
})(typeof window !== "undefined" ? window : global); // Attach to window in browser

// Blop Component (using the embedded noise library)
const getNoise = (x, y) => {
  const noiseFn = window?.noise?.simplex2;
  if (typeof noiseFn !== "function") {
    console.warn("Noise function not available - using fallback (static blob)");
    return 0; // Fallback: No noise, static shape
  }
  return noiseFn(x, y);
};
