...
const fetchVerificationList = async () => {
  // Note: Ensure this JSON file exists in /public/data/ or API route
  ...
  if (!response.ok) {
    if (response.status === 404) {
      console.log("Verification file not found, returning empty list.");
      return [];
    }
    throw new Error(`Failed to fetch verification list: ${response.statusText}`);
  }
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <p className="font-bold text-mainColor mb-4 text-base md:text-lg">
              Membership is free and open to all users.
            </p>
            <p className="text-black mb-4 text-base md:text-lg">
              To pass verification please provide your <span className="text-md font-bold text-mainColor">full name and appropriate username.</span>
            </p>
