...
const verifyAttendee = async ({ eventId, attendeeUserId, verified }) => {
    const response = await fetch("/api/events", {
        ...
        body: JSON.stringify({ eventId, attendeeUserId, verified }),
    });
    if (!response.ok) {
    ...
    return { eventId, attendeeUserId, verified };
};
...
export default function EditEventModal({
          ...
          queryClient.setQueryData(['events'], (oldData) => {
              ...
              return oldData.map(event => {
                  if (event.id === data.eventId) {
                      const updatedAttendees = (event.attendeeDetails || []).map(att =>
                          att.userID === data.attendeeUserId ? { ...att, verified: data.verified } : att
                      );
                      const updatedCounter = (event.attendeesCounter || []).map(att =>
                          att.userID === data.attendeeUserId ? { ...att, verified: data.verified } : att
                      );
                      return { ...event, attendeeDetails: updatedAttendees, attendeesCounter: updatedCounter };
                  }
                  return event;
              });
          });
          console.log(`Verification status updated for user ${data.attendeeUserId}`);
          setError("");
      },
      onError: (error) => {
  ...
  useEffect(() => {
    if (eventData) {
      ...
      setAttendees(typeof eventData.attendees === 'boolean' ? eventData.attendees : false);
      setCardColor(eventData.cardColor || "bg-white");
      setIsLimitEnabled(typeof eventData.isLimitEnabled === 'boolean' ? eventData.isLimitEnabled : false);
      setLimitValue(typeof eventData.attendanceLimit === 'number' ? eventData.attendanceLimit : 0);
      setCardEnabled(typeof eventData.cardEnabled === 'boolean' ? eventData.cardEnabled : true);
      setInDescription(eventData.inDescription && eventData.inDescription.length > 0 ? eventData.inDescription : [{ title: "", description: "" }]);
      setError("");
      setActiveTab("main");
    } else {
       ...
       setCardEnabled(true); setInDescription([{ title: "", description: "" }]);
  ...
  const handleResetAttendees = () => {
    if (!eventData || !eventData.id) { setError("Cannot reset attendees: Event data is missing."); return; }
  ...
  const handleSave = (resetAttendeesFlag = false) => {
    ...
    if (!eventData || !eventData.id) { setError("Cannot save event: Original event data is missing."); return; }
    ...
    const updatedEventData = { id: eventData.id, title, date: date.toISOString(), description, location, imageUrl, attendees, cardColor, isLimitEnabled, attendanceLimit: isLimitEnabled ? limitValue : null, cardEnabled, resetAttendees: resetAttendeesFlag, inDescription: finalInDescription, };
  ...
  const handleFreezeClick = () => {
      ...
      if (confirm("Are you sure you want to freeze all currently unverified attendees for this event? This will prevent them from attending future events until unfrozen.")) {
          setError("");
          freezeMutation.mutate({ eventId: eventData.id });
      }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 max-h-[calc(90vh-160px)] overflow-y-auto">
            <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-4 md:border-r">
              ...
              <div className="pt-4 flex justify-end space-x-3 border-t mt-4">
                  <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition" disabled={editEventMutation.isPending || freezeMutation.isPending}>Cancel</button>
                  <button type="submit" className={`px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition ${ (editEventMutation.isPending || freezeMutation.isPending) ? "opacity-50 cursor-not-allowed" : "" }`} disabled={editEventMutation.isPending || freezeMutation.isPending}>{editEventMutation.isPending ? "Saving..." : "Save Changes"}</button>
              </div>
            </form>
            <div className="p-4 md:p-6 space-y-3">
                <div className="flex justify-between items-center border-b pb-2">
                    <h3 className="text-lg font-medium text-gray-800">Attendees ({attendeesList.length})</h3>
                    <button
                      type="button"
                      onClick={handleResetAttendees}
                      className={`px-3 py-1 bg-red-100 text-red-700 rounded-md hover:bg-red-200 border border-red-300 text-xs transition ${editEventMutation.isPending || freezeMutation.isPending ? "opacity-50 cursor-not-allowed" : ""}`}
                      disabled={editEventMutation.isPending || freezeMutation.isPending || !attendees}
                ...
                {attendeesList.length > 0 ? (
                    <ul className="space-y-1 text-sm text-gray-600 max-h-48 overflow-y-auto border rounded p-2">
                        {attendeesList.map((attendee) => (
                            <li key={attendee.userID} className="flex items-center justify-between px-2 py-1.5 bg-gray-50 rounded">
                                <span className="break-words mr-2">{attendee.fullName}</span>
                                <button
                                    type="button"
                                    onClick={() => handleVerifyToggle(attendee.userID, attendee.verified)}
                                    disabled={verifyAttendeeMutation.isPending && verifyAttendeeMutation.variables?.attendeeUserId === attendee.userID}
                                    className={`p-1 rounded-full transition-colors ${attendee.verified ? 'bg-green-100 text-green-600 hover:bg-green-200' : 'bg-gray-200 text-gray-500 hover:bg-gray-300'} ${verifyAttendeeMutation.isPending && verifyAttendeeMutation.variables?.attendeeUserId === attendee.userID ? 'opacity-50 cursor-wait' : ''}`}
                ...
                {attendees && attendeesList.length > 0 && hasUnverified && (
                    <div className="mt-4 pt-4 border-t">
                        <button
                            ...
                            className={`w-full px-4 py-2 bg-yellow-100 text-yellow-800 rounded-md hover:bg-yellow-200 border border-yellow-300 text-sm transition ${freezeMutation.isPending ? "opacity-50 cursor-not-allowed" : ""}`}
